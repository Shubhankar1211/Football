...
function PlayerCard({ player, onDecrementScore, showAll}) {
  ...
  return (
    <div className="flex flex-col p-4 my-auto w-full justify-evenly">
      <img src={player.imageURL} className={'mx-auto rounded-lg w-1/3 my-2 ' + (showAll ? "" : (imgBlur == 1 ? "blur-[4px] cursor-pointer" : "blur-none"))} onClick={unBlur} />
  ...
  let infos = [
    [
      ...
      },
      {
        key: 4,
        lib_value: "Foot",
        value: player.foot.charAt(0).toUpperCase() + player.foot.slice(1).toLowerCase(),
        hidden: showAll ? 0 : 1
      }
    ],
    [
